...
import RoomDesingCard from './RoomDesignCard';
...
function Listing() {
    ...
    const GetUserRoomList = async () => {
        const result = await db.select().from(AiGeneratedImage).where(eq(AiGeneratedImage.userEmail, user?.primaryEmailAddress?.emailAddress)).orderBy(desc(AiGeneratedImage.id))
        ...
        <div>
            ...
            <div className="grid grid-cols-3 gap-4">
                {userRoomList.map((room, index) => (
                    ...
                    <RoomDesingCard room={room}></RoomDesingCard>
                    </div>
                ))}
            </div>
        }
